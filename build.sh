...
cd $KERNEL_DIR
#git clone https://github.com/rama982/AnyKernel2 -b daisy-miui

# Exports
export ARCH=arm64
export SUBARCH=arm64
export KBUILD_BUILD_USER="ahmedhady"
export PATH=/usr/lib/ccache:$PATH

# Misc
CONFIG=daisy_defconfig
CORES=$(grep -c ^processor /proc/cpuinfo)
THREAD="-j16" #$CORES"

# Main script
while true; do
echo -e "\n$green[1] Build Daisy P Kernel (with Google GCC)"
...
if [ "$choice" == "2" ]; then
  ...
  cp out/.config arch/arm64/configs/daisy-full_defconfig
...
if [ "$choice" == "4" ]; then
echo -e "\n$green#######################################################################$nc"
echo -e "\n$green Strip and move modules to AnyKernel2..."
...
sed -e 's/gcc/strip/')"
  ...
  cp $DTB_T $ZIP_DIR/treble/msm8953-qrd-sku3-daisy.dtb
